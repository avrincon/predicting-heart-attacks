...
lr_age <- 
  glm(
    result ~ age + log_ck_mb + log_troponin,
    family = binomial(link = "logit"),
    data = d3
  )
```
...
```{r}
anova(logistic_model, lr_age, test = "Chisq")
anova(logistic_model, reduced_model, test = "Chisq")
anova(reduced_model, lr_age, test = "Chisq")
```
...
fit_summary <- 
  ...
  mutate(
    Model = c("Full Model", "Reduced Model", "Age Model"), 
    .before = 1
  ) |> 
...
Age has an apparent, perhaps un-intuative, negative relationship with heart attack risk. However, biomarkers like troponin and CK-MB are the true drivers of heart attack risk. Older people tend to have higher levels of these biomarkers, which creates an apparent relationship between age and heart attacks. When we control for the biomarker levels directly, we find that for people with the same biomarker levels, younger people might actually have a slightly higher risk (hence the negative coefficient). Having said that, the reduced model has a smaller AIC and BIC, and has the same predictive power as the full model, suggesting that age is overall not a significant predictor of heart attacks.
...
troponin_grid <- exp(
  seq(log(0.001), log(max(d3$troponin)), length.out = 100)
)
ckmb_grid <- exp(
  seq(log(0.3), log(max(d3$ck_mb)), length.out = 100)
)
```
...
pred_data <- 
  tibble(
    ...
    log_ck_mb = quantile(d3$log_ck_mb, probs = 0.1),                       
...
pred_ck_mb <- 
  tibble(
    ...
    diastolic_blood_pressure = mean(d3$diastolic_blood_pressure),
    blood_sugar = mean(d3$blood_sugar),          
    log_troponin = quantile(d3$log_troponin, probs = 0.10),
    log_ck_mb = seq(min(d3$log_ck_mb), max(d3$log_ck_mb), length.out = 100), 
...
pred_age <- 
  tibble(
    ...
    # age_group = factor(levels(d3$age_group)),              
    gender = "Male",                     
    heart_rate = mean(d3$heart_rate),
    systolic_blood_pressure = mean(d3$systolic_blood_pressure),  
    diastolic_blood_pressure = mean(d3$diastolic_blood_pressure),
    blood_sugar = mean(d3$blood_sugar),          
    log_troponin = quantile(d3$log_troponin, probs = 0.5),
    log_ck_mb = quantile(d3$log_ck_mb, probs = 0.25), 
...
# pred_age |> 
#   ggplot(aes(x = age_group, y = predicted_prob)) +
